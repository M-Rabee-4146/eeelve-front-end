...
import { RemoveItem } from "../Redux_toolkit/Features/Cart"
import { useState } from "react"
import { Heart, MapPin, Menu, Phone, Search, SearchIcon, ShoppingCart, User, User2, X } from "lucide-react"
...
export default function Navbar() {
  ...
  const [togleSidebar, SetToggleSidebar] = useState(false)
    if(togleSidebar==true){
      document.querySelector('.my-html').classList.add('overflow-y-hidden')
    }
    else{
      document.querySelector('.my-html').classList.remove('overflow-y-hidden')
    }
  ...
  return (
    <div className="helo relative">
      <CartSidebar
      sidebar={togleSidebar}
      setsidebar={SetToggleSidebar}
      totalprc={getTotal().totalprice}/>
      ...
      <div className="links w-full h-10 hidden justify-between md:flex">
        ...
        <div className="right flex items-center">
          ...
          <Link to={'/api/Login'} >    <span className="text-gray-600 mr-1 text-sm hover:text-amber-400 transition-all duration-300  ">Login  </span></Link><span className="text-gray-600"> Or</span>
          ...
          <div className="flex justify-between h-16  ">
            ...
            <div className="hidden md:flex items-center space-x-4">
              <button variant="outline" size="sm" asChild>
                <Link to={'/api/Register'}><User className="size-5" /></Link>
              </button>
              <button size="sm" asChild>
                <Heart className="size-5" />
              </button>
              <Link href="/cart" onClick={() => SetToggleSidebar(true)} className="flex items-center relative text-gray-700 hover:text-primary transition-colors duration-500">
                <ShoppingCart className="h-5 w-5 mr-1 " />
                <div className="quantity bottom-3 left-3 absolute bg-amber-400 size-4 rounded-xl flex items-center justify-center">

                  <p className="  text-sm">{getTotal().totalqty || 0}</p>
                </div>
                {/* <span>Cart</span> */}
              </Link>
            </div>
            ...
            <div className="flex md:hidden items-center justify-between w-full">
              ...
              <div className="right-small flex">
                ...
                <Link to={'/api/Register'} href="/api/Login" className="flex items-center text-gray-700 hover:text-primary transition-colors mr-4 duration-500">
          ...
          <div className="md:hidden">
            <div className="   pt-2 pb-3 space-y-1 px-3">
              ...
              <div className="flex flex-col space-y-3">
                ...
                <a
                  href="/signup"
                  className="flex items-center Fpx-3 py-2 bg-primary text-black rounded-md hover:bg-primary/90 transition-colors"
                >
                  Sign Up
                </a>
              </div>
